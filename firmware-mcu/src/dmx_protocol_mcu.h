 ...
 *
 * Binary protocol for RPMSG communication between Linux and MCU.
 */

#ifndef DMX_PROTOCOL_MCU_H
#define DMX_PROTOCOL_MCU_H
...
typedef enum {
    ...
    /* System commands (MCU only - require magic validation) */
    CMD_SYSTEM_RESET        = 0xFF,  /* Graceful MCU reset (requires SYSTEM_RESET_MAGIC) */
} dmx_cmd_type_t;

/* ============================================================================
 * System Command Security (MCU only)
 * ============================================================================ */

/*
 * Magic value for CMD_SYSTEM_RESET
 *
 * The reset command requires this 4-byte magic in the payload to prevent
 * accidental resets. This is used for graceful shutdown before Linux reboot.
 *
 * Payload format: [magic:4] = 0xDEADBEEF (little-endian: EF BE AD DE)
 */
#define SYSTEM_RESET_MAGIC      0xDEADBEEFU
